...
const CANDIDATES = Object.freeze([
  { presetName: "中性测评·四月", fileName: "918_V5-A中性测评_四月_Qwen试听.wav" },
  { presetName: "率性变音·月白", fileName: "918_V5-B率性变音_月白_Qwen试听.wav" },
  { presetName: "设计师变音·不吃鱼", fileName: "918_V5-C设计师变音_不吃鱼_Qwen试听.wav" }
]);
...
async function main() {
  ...
  const outputDir = path.resolve(process.argv[2] || path.join(process.cwd(), "output", "speech", "neutral-voice-previews"));
  await fs.mkdir(outputDir, { recursive: true });
  const results = [];
  for (const candidate of CANDIDATES) {
    const outputPath = path.join(outputDir, candidate.fileName);
    const synthesis = await synthesizeQwenVoice(PREVIEW_TEXT, outputPath, {
      apiKey,
      presetName: candidate.presetName,
      duration: 29
    });
    results.push({ ...synthesis, instructions: undefined });
  }
